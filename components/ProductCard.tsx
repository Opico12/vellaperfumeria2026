                        ...
                        <div className="flex justify-center gap-2 overflow-x-auto no-scrollbar py-2 px-1 scroll-smooth snap-x">
                            {product.variants![type].map(opt => (
                                <button
                                    ...
                                    className={`w-7 h-7 rounded-full border-2 transition-all flex-shrink-0 snap-center ${selectedVariant?.[type] === opt.value ? 'border-black scale-125 ring-2 ring-pink-100' : 'border-white hover:scale-110 shadow-sm'}`}
                                    style={{ backgroundColor: opt.colorCode }}
                                    title={opt.value}
                                />
                            ))}
                        </div>
                        <p className="text-[8px] font-black text-pink-600 uppercase tracking-widest mt-2">{selectedVariant?.[type]}</p>
                    </div>
                ))}
