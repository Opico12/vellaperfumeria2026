...
const HomeIcon = ({ isActive }: { isActive: boolean }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isActive ? 2.5 : 2}>
...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ onNavigate, currentView, currentCategory }) => {
    ...
    const navItems: NavItem[] = [
        ...
        { view: 'skinDiagnostic', label: 'Piel', icon: HelpIcon, payload: undefined },
    ];

    return (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-black border-t border-white/5 shadow-2xl z-[90] pb-safe">
                ...
                {navItems.map(item => {
                    ...
                    return (
                        <button
                            key={item.label}
                            onClick={handleClick}
                            className={`flex flex-col items-center justify-center text-[8px] font-black uppercase tracking-[0.1em] w-full h-full transition-all ${
                                isActive ? 'text-[#FAE1EF]' : 'text-gray-400'
                            }`}
                            aria-label={item.label}
                            aria-current={isActive ? 'page' : undefined}
                        >
                            <div className={`p-1.5 rounded-full transition-colors ${isActive ? 'bg-white/10' : ''}`}>
